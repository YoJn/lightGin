...
func iterate(path, method string, routes RoutesInfo, root *node) RoutesInfo {
	...
	if len(root.handlers) > 0 {
		...
		routes = append(routes, RouteInfo{
			Method:      method,
			Path:        path,
			Handler:     common.NameOfFunction(handlerFunc),
			HandlerFunc: handlerFunc,
		})
	}
...
func serveError(c *Context, code int, defaultMessage []byte) {
	c.writerEntity.status = code
	c.Next()
	if c.writerEntity.Written() {
		return
	}
	if c.writerEntity.Status() == code {
...
func redirectFixedPath(c *Context, root *node, trailingSlash bool) bool {
	...
	if fixedPath, ok := root.findCaseInsensitivePath(common.CleanPath(rPath), trailingSlash); ok {
		req.URL.Path = common.BytesToString(fixedPath)
		redirectRequest(c)
		return true
	}
	return false
